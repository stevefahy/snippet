...
function Exposure2Filter(){
	...
	this.filter = function(input,values){
		...
		for(var i = 0; i < 256; i++){
			...
			//table[i] = i * (Math.pow(2, amount));
			//table[i] = table[i] * (Math.pow(2, amount));
			//out = (2 * in ^ gamma) ^ (1 / gamma),
			//table[i] = Math.pow(2 * (Math.pow(i,table[i])), (1/table[i]));
			//table[i] = Math.pow(i, (amount*.1));

			//table[i] = parseInt(255 *(1-Math.exp(-(i/255) * exposure)),10);
				
			//var g = 255 * Math.pow(i/255, 1/amount) + 0.5;
			//var val2 = Math.min(255 *(1-2*(-(i/255) * amount)),RGBmax);
			...
			//var val = Math.pow(g, amount);
			
				table[i] = val2;
			
			
			//table[i] = Math.pow(table[i], amount);
		}
		filterUtils.tableFilter(inputData,table,width,height);
	};
}
